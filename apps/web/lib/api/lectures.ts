...
export const getLecturesByCourse = (courseId: string) =>
  createClient().from('lectures').select('*').eq('course_id', courseId).order('order_index');

export const getLectureById = (id: string) =>
  createClient().from('lectures').select('*').eq('id', id).single();

export const createLecture = (lecture: LectureInsert) =>
  createClient().from('lectures').insert(lecture).select().single();

export const updateLecture = (id: string, updates: LectureUpdate) =>
  createClient().from('lectures').update(updates).eq('id', id);

export const deleteLecture = (id: string) =>
  createClient().from('lectures').delete().eq('id', id);

// Re-export row type for use in components
export type { LectureRow, LectureInsert, LectureUpdate };
